...
int main()
{
	...
	*/
	...
	while (option != 4) {
		...
		switch (option) {
			case 1:
				// call function to print all items and the number of ocurrences
				CallProcedure("print_all_items");
				cout << endl;
				break;
			case 2:
				// assign searchItem with string from read String function
				searchItem = console.readString("What item would you like to search for? ");
				// call function to count all ocurrences of specific item and send userItem as argument
				callIntFunc("count_all_ocurrences", searchItem);
				cout << endl;
				break;
			case 3:
